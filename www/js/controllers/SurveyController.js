surveyApp.controller('SurveyController', function ($rootScope, $scope, $state, $http,
     AuthService, SurveyService, Dealers, AUTH_EVENTS, NETWORK, DealerMap) {
    ...
    $rootScope.$on('$stateChangeStart', function (event, toState, toParams, fromState, fromParams) {
        ...
        //else if (fromState.name == "tabs.survey" && toState.name == "tabs.newsurvey") {
        //    $scope.setTabVisibility('none');
        //    console.log("hidden");
        //}
    });
    $scope.update = false;
    ...
    function onSuccess(imageURI) {
        $scope.$apply(function () {
            ...
            switch ($scope.id) {
                case 1:
                    img = document.getElementById('img_cmnd1');
                    img.src = imageURI
                    $scope.imgCMND1 = imageURI;
                    $scope.setUpdateImage($scope.id);
                    break;
                case 2:
                    img = document.getElementById('img_cmnd2');
                    img.src = imageURI
                    $scope.imgCMND2 = imageURI;
                    $scope.setUpdateImage($scope.id);
                    break;
                case 3:
                    img = document.getElementById('img_dealer');
                    img.src = imageURI
                    $scope.imgDealer = imageURI;
                    $scope.setUpdateImage($scope.id);
                    break;
                case 4:
                    img = document.getElementById('img_shop');
                    img.src = imageURI
                    $scope.imgShop = imageURI;
                    $scope.setUpdateImage($scope.id);
                    break;
                case 5:
                    img = document.getElementById('img_warehouse');
                    img.src = imageURI
    ...
    $scope.setTabVisibility = function(flag){
        document.getElementById("tab").style.display = flag;
    }
});